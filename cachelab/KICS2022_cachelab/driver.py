...
#     matrices (32x32, 61x67, and 64x64) to test the correctness and
...
#            print "%s" % (line)
    ...
    # 32x32 transpose
    print("Testing the transpose function")
...
#    csim_cscore  = [0] #map(int, resultsim[0:1])
...
#    total_score = csim_cscore[0] + trans32_score + trans64_score + trans61_score
    ...
    print("\nCache Lab summary:")
    print(f"{' ':<24}Points{' ':<3}Max pts{' ':<6}Misses")
...
#                              maxscore['csim'])
    ...
    if miss32 == 2**31-1 :
        misses = "invalid"
    print(f"Trans perf 32x32{' ':<11}{trans32_score:<12.1f}"
          f"{maxscore['trans32']:<6}{misses:<5}")

    misses = str(miss61)
    if miss61 == 2**31-1 :
        misses = "invalid"
    print(f"Trans perf 61x67{' ':<11}{trans61_score:<11.1f}"
          f"{maxscore['trans61']:<7}{misses:<5}")

    misses = str(miss64)
    if miss64 == 2**31-1 :
        misses = "invalid"
    print(f"Trans perf 64x64{' ':<11}{trans64_score:<12.1f}"
          f"{maxscore['trans64']:<6}{misses:<5}")
